			...
			new swaggerNodeClient({
				...
				.then(async (client) => {
					for (const id of orcidIDs) {
						...
						await client.apis['Public API v2.0']
							.viewWorks({ orcid: id }, { responseContentType: 'application/JSON' })
							.then(async (works) => {
								try {
									await miembroOrcid.findOneAndUpdate(
										...
										(err, doc) => {
											if (err) {
												console.log(
													`Error al intentar limpiar lista publicaciones miembro: ` + err
												);
											} else {
												console.log(
													doc.miembro.orcidID + ' con ninguna publicación aún asociada'
												);
											}
										}
									);
										...
										for (let datos2 in works[datos1].group) {
											if (works[datos1].group[datos2].hasOwnProperty('work-summary')) {
												...
												for (let datos3 in works[datos1].group[datos2]['work-summary']) {
													if (
														works[datos1].group[datos2]['work-summary'][datos3][
															'publication-date'
														] !== null
													) {
														if (
															...
															if (
																...
																for (let datos4 in works[datos1].group[datos2][
																	'work-summary'
																][datos3]['external-ids']['external-id']) {
																	doieid.push(
																		works[datos1].group[datos2]['work-summary'][
																			datos3
																		]['external-ids']['external-id'][datos4][
																			'external-id-value'
																		]
																	);
																}
																			...
																			try {
																				await miembroOrcid.findOneAndUpdate(
																					...
																					(err, doc) => {
																						if (err) {
																							console.log(
																								`Error al actualizar lista publicaciones miembro tras encontrarse trabajo repetido: ` +
																									err
																							);
																						} else {
																							console.log(
																								doc.miembro.orcidID +
																									' actualizado correctamente al encontrarse una publicación existente anteriormente'
																							);
																						}
																					}
																				);
																				...
																				console.log(
																					'Error al intentar insertar id de trabajo repetido en publicaciones miembro Orcid'
																				);
																			}
																		};

																		publicacionOrcid.find(
																			{ 'publicacion.titulo': trabajo.titulo },
																			async (error, trabajo3) => {
																				error
																					? console.log(error)
																					: await actualizaRepetido(trabajo3);
																			}
																		);
																	} else {
																		...
																		try {
																			await miembroOrcid.findOneAndUpdate(
																				...
																				(err, doc) => {
																					if (err) {
																						console.log(
																							`Error al actualizar lista publicaciones miembro tras encontrar trabajo nuevo: ` +
																								err
																						);
																					} else {
																						console.log(
																							doc.miembro.orcidID +
																								' actualizado correctamente tras trabajo nuevo'
																						);
																					}
																				}
																			);
																			...
																			console.log(
																				'Error al intentar insertar id trabajo nuevo en publicaciones miembro Orcid'
																			);
																		}
																	}
																});
...
async function getAllMemberWorks(req, res, next) {
	...
	var obtenerTrabajosAño = async (fechas2) => {
		for (let fecha in fechas2) {
			try {
				await publicacionOrcid.find(
					{ 'publicacion.fecha': fechas2[fecha] },
					{
...
async function getOneMemberWorks(req, res, next) {
	//en lugar de usar esta function getOneMemberWorks(req,res) para poder usarlo en el fichero de rutas
		...
		.exec(async (err, miembro) => {
			...
			if (err) {
				...
				if (publFechas.length < 1) {
					publFechas = 'Este miembro no cuenta con publicaciones';
                }
...
function getAllJournalMemberWorks2018(req, res, next) {
	...
	const getWorks = async (id, client) => {
		return await client.apis['Public API v2.0'].viewWorks(
			{ orcid: id },
												...
												for (let datos3 in works[datos1].group[datos2]['work-summary']) {
													if (
														...
														if (
															...
															works[datos1].group[datos2]['work-summary'][datos3].type ==
																'JOURNAL_ARTICLE'
														) {
															let doieid = [];
															for (let datos4 in works[datos1].group[datos2][
																...
																doieid.push(
																	works[datos1].group[datos2]['work-summary'][datos3][
																		'external-ids'
																	]['external-id'][datos4]['external-id-value']
																);
															}
															trabajo = {
																titulo:
